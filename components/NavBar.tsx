  ...
  return (
    <>
      <style>{`
        ...
        .vela-user-name{font-size:14px;font-weight:600;color:#333D4B;text-decoration:none;transition:color .15s}
      ...
      <nav className="vela-nav">
        <div className="vela-nav-inner">
          ...
          <div className="vela-nav-actions">
            {user ? (
              <>
                <Link href="/profile" className="vela-user-name">{user.user_metadata?.full_name ?? user.email?.split("@")[0] ?? "내 계정"}</Link>
                <Link href="/dashboard" className="vela-btn-login">월별현황</Link>
                <Link href="/simulator" className="vela-btn-start">시뮬레이터 →</Link>
                <Link href="/profile" className="vela-btn-login">내정보</Link>
      ...
      <div className={`vela-mobile-menu${menuOpen ? " open" : ""}`}>
        ...
        {user ? (
          <>
            ...
            <Link href="/profile" className="vela-mobile-link" onClick={() => setMenuOpen(false)}>내 정보</Link>
