...
const INDUSTRY_LABELS: Record<string, string> = {
  cafe: "카페", restaurant: "음식점", bar: "술집/바", finedining: "파인다이닝",
};
...
// ─── 피드 탭 ───────────────────────────────────────────────────
type SharePost = {
  id: string; nickname: string; industry: string; title: string;
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {posts.map(p => <FeedCard key={p.id} post={p} onLike={() => handleLike(p.id)} />)}
        </div>
      )}
    </div>
  );
...
function FeedCard({ post: p, onLike }: { post: SharePost; onLike: () => void }) {
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-lg">{INDUSTRY_ICONS[p.industry] ?? "🏪"}</span>
            <span className="text-xs font-semibold text-slate-400">{INDUSTRY_LABELS[p.industry] ?? p.industry}</span>
          </div>
        ...
        <div className={`shrink-0 rounded-2xl px-3 py-2 text-center ${isProfit ? "bg-emerald-50" : "bg-red-50"}`}>
...
// ─── 게시판 탭 ─────────────────────────────────────────────────
type BoardPost = {
  id: string; nickname: string; category: string; industry: string;
